...
var AtomicRule = class {
  ...
  normalize = (styles, normalizeShorthand = true) => {
    return normalizeStyleObject(styles, this.context, normalizeShorthand);
  };
  process = (options) => {
...
// src/serialize.ts
import { filterBaseConditions as filterBaseConditions2, isImportant as isImportant2, normalizeStyleObject as normalizeStyleObject2, walkObject as walkObject2, withoutImportant as withoutImportant2 } from "@pandacss/shared";
import merge from "lodash.merge";
function serializeStyle(styleObj, context) {
  const { utility, conditions } = context;
  const rule = conditions.rule();
  const result = {};
  const normalizedObject = normalizeStyleObject2(styleObj, context);
...
var Layers = class {
  constructor(names) {
    this.names = names;
    this.root = postcss7.root();
    this.reset = postcss7.atRule({ name: "layer", params: names.reset, nodes: [] });
    this.base = postcss7.atRule({ name: "layer", params: names.base, nodes: [] });
    this.tokens = postcss7.atRule({ name: "layer", params: names.tokens, nodes: [] });
    this.recipes = {
      root: postcss7.atRule({ name: "layer", params: names.recipes, nodes: [] }),
      base: postcss7.atRule({ name: "layer", params: "_base", nodes: [] })
    };
    this.slotRecipes = {
      root: postcss7.atRule({ name: "layer", params: names.recipes + ".slots", nodes: [] }),
      base: postcss7.atRule({ name: "layer", params: "_base", nodes: [] })
    };
    this.utilities = {
      root: postcss7.atRule({ name: "layer", params: names.utilities, nodes: [] }),
      compositions: postcss7.atRule({ name: "layer", params: "compositions", nodes: [] }),
      custom: (layer) => {
        if (!this.utilityRuleMap.has(layer)) {
          const atRule3 = postcss7.atRule({ name: "layer", params: layer, nodes: [] });
  ...
  getLayer(layer) {
    ...
    switch (layer) {
      ...
      case "recipes": {
        const recipeRoot = postcss7.root();
...
var expandToken = (str, replacer, getterFn) => {
  const value = str.replace(tokenRegex, (_, token) => {
    ...
    const result = [tokenValue, tokenFallback].filter(Boolean).map((s) => getterFn?.(s) ?? esc2(s));
    if (result.length > 1) {
      const [a, b] = result;
      return replacer(a, b);
    }
...
function optimizeCss(code, options = {}) {
  ...
  const { css } = postcss8(plugins).process(code);
  return css;
}
function expandCssFunctions(code, options = {}) {
  const { token, raw } = options;
  const { css } = postcss8([expand_token_fn_default(token, raw)]).process(code);
  return css;
}
function expandNestedCss(code) {
  const { css } = postcss8([nested(), prettify()]).process(code);
  return css;
}
function prettifyCss(code) {
  const { css } = postcss8([prettify()]).process(code);
  return css;
}

// src/recipes.ts
import { capitalize as capitalize2, createRegex, dashCase, getSlotRecipes, memo, splitProps } from "@pandacss/shared";
...
var Recipes = class {
  ...
  assignRecipe = (name, recipe) => {
    ...
    const capitalized = capitalize2(name);
    const jsx = recipe.jsx ?? [capitalized];
    if ("slots" in recipe) {
      jsx.push(...recipe.slots.map((slot) => capitalized + "." + capitalize2(slot)));
    }
...
import { getSlotRecipes as getSlotRecipes2 } from "@pandacss/shared";
