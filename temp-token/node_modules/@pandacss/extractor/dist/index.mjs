...
function getDeclarationFor(node, stack, ctx) {
  ...
  let declaration;
  if ((Node3.isVariableDeclaration(parent) || Node3.isParameterDeclaration(parent) || Node3.isFunctionDeclaration(parent) || Node3.isEnumDeclaration(parent) || Node3.isBindingElement(parent)) && parent.getNameNode() == node) {
...
var getPropertyName = (property, stack, ctx) => {
  ...
  if (Node4.isPropertyAssignment(property)) {
    ...
    if (Node4.isIdentifier(node))
      return box.from(node.getText(), node, stack);
    if (Node4.isComputedPropertyName(node)) {
      const expression = node.getExpression();
      stack.push(expression);
      return maybePropName(expression, stack, ctx);
    }
    if (Node4.isStringLiteral(node) || Node4.isNumericLiteral(node))
...
function maybeBoxNode(node, stack, ctx, matchProp) {
  ...
  if (Node6.isStringLiteral(node) || Node6.isNoSubstitutionTemplateLiteral(node)) {
    const value = trimWhitespace(node.getLiteralValue());
    return cache(box.literal(value, node, stack));
  }
  if (Node6.isObjectLiteralExpression(node)) {
    return cache(getObjectLiteralExpressionPropPairs(node, stack, ctx, matchProp));
  }
  if (Node6.isTrueLiteral(node) || Node6.isFalseLiteral(node)) {
    const value = node.getLiteralValue();
    return cache(box.literal(value, node, stack));
  }
  if (Node6.isNumericLiteral(node)) {
    const value = node.getLiteralValue();
    return cache(box.literal(value, node, stack));
  }
  if (Node6.isNullLiteral(node)) {
    return cache(box.literal(null, node, stack));
  }
  if (Node6.isPrefixUnaryExpression(node)) {
  ...
  if (Node6.isArrayLiteralExpression(node)) {
  ...
  if (Node6.isConditionalExpression(node)) {
    ...
    if (box.isEmptyInitializer(condValue))
      return;
    const isFromDefaultBinding = condValue.getStack().some((node2) => Node6.isBindingElement(node2));
  ...
  if (Node6.isBinaryExpression(node)) {
...
function maybePropDefinitionValue(def, accessList, _stack, ctx) {
  ...
  if (Node6.isEnumDeclaration(def)) {
...
var extractJsxAttribute = (jsxAttribute, ctx) => {
  ...
  if (Node7.isJsxExpression(initializer)) {
...
// src/extract.ts
var isImportOrExport = (node) => Node8.isImportDeclaration(node) || Node8.isExportDeclaration(node);
var isJsxElement = (node) => Node8.isJsxOpeningElement(node) || Node8.isJsxSelfClosingElement(node);
var extract = ({ ast, ...ctx }) => {
...
var getLiteralValue = (node, ctx) => {
  ...
  if (box.isConditional(node)) {
    ...
    if (last && Node10.isArrayLiteralExpression(last) && !Number.isNaN(maybeIndex)) {
