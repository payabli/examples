...
var dom = (0, import_dom_query.createScope)({
  ...
  getFocusedCell: (ctx, view = ctx.view) => (0, import_dom_query.query)(dom.getContentEl(ctx), `[data-part=cell-trigger][data-type=${view}][data-focused]:not([data-outside-range])`),
...
function connect(state, send, normalize) {
  ...
  const api = {
    ...
    getDaysInWeek(weekIndex, from = startValue) {
      return (0, import_date_utils.getDaysInWeek)(weekIndex, from, locale, startOfWeek);
    },
    getOffset(months) {
    ...
    visibleRange: state.context.visibleRange,
    getYears() {
      return (0, import_date_utils.getDecadeRange)(focusedValue.year).map((year) => ({ label: year.toString(), value: year }));
    },
    getYearsGrid(props = {}) {
      const { columns = 1 } = props;
      return (0, import_utils.chunk)(api.getYears(), columns);
    },
    getDecade() {
    ...
    getMonthsGrid(props = {}) {
      ...
      return (0, import_utils.chunk)(api.getMonths({ format }), columns);
    },
...
var getInitialContext = (ctx) => {
  ...
  let focusedValue = value.at(0) || ctx.focusedValue || (0, import_date_utils2.getTodayDate)(timeZone);
  ...
  return (0, import_core.createMachine)(
    {
      ...
      computed: {
        valueText: (ctx2) => ctx2.value.map((date) => (0, import_date_utils2.formatSelectedDate)(date, null, ctx2.locale, ctx2.timeZone)).join(", "),
...
var set = {
  ...
  view(ctx, value) {
    if ((0, import_utils2.isEqual)(ctx.view, value))
      return;
    ctx.view = value;
